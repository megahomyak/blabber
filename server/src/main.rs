...
fn main() {
    ...
    rouille::start_server("0.0.0.0:80", move |request| {
        ...
        while let Some(new_message) = ass
            .lock()
            .unwrap()
            .get(format!("{}/messages/{}", request.room_name, current_message_id).as_bytes())
        {
        ...
        if !request.self_message_lines.is_empty() {
            let new_message_id = ass
                .lock()
                .unwrap()
                .get(format!("{}/last message id", request.room_name).as_bytes());
