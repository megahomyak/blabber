...
fn main() {
    ...
    let server = rouille::Server::new("0.0.0.0:80", move |request| {
        ...
        while let Some(new_message) =
            ass.lock().unwrap().get(format!("{}/messages/{}", request.room_name, current_message_id).as_bytes())
        {
        ...
        if !request.self_message_lines.is_empty() {
            let new_message_id =
                ass.lock().unwrap().get(format!("{}/last message id", request.room_name).as_bytes());
    ...
    std::thread::spawn(move || {
        for _sig in signal_hook::iterator::Signals::new([signal_hook::consts::SIGTERM, signal_hook::consts::SIGINT, signal_hook::consts::SIGQUIT]).unwrap().forever() {
            stopper.send(()).unwrap();
        }
    });
